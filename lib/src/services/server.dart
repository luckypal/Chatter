...
abstract class ServerService {
  Future<Map<String, dynamic>> uploadProfileImage(
      String phoneNumber, String name, File image);
  Future<void> findPhones(List<String> phoneNumbers);
}

class ServerServiceImpl extends ServerService {
  @override
  Future<Map<String, dynamic>> uploadProfileImage(
      String phoneNumber, String userName, File image) {
  ...
  Future<void> findPhones(List<String> phoneNumbers) {
    return new Future(() async {
      try {
        String url = '${config.Server.SERVER_URL}/find/phones';
        final httpRequest = http.Request('POST', Uri.parse(url));
        httpRequest.body = json.encode(phoneNumbers);
